...
This is an exercise that works best with a small group of people working from a single git repo. The aim is to demonstrate in a simple fashion how Github treats file merges, and how merge conflicts arise.
...
## Round :three: Taking it further...
1. Choose an application that a member of your group already has written.

2. Run the application to verify the expected behavior.

3. Working collaboratively, write a Dockerfile for the application, build an image for the new application, and test the image by creating a container.

4. Add the Dockerfile and project to the repo on a **new separate feature branch**, then create a pull request to the main branch of the repo.  Another member of the group should review it and approve it!

5. Create a GitHub Actions workflow (CI/CD pipeline) that will:
  - run whenever new code is pull-requested into main, or whenever manually triggered.
  - use the Dockerfile to build a Docker image of the groups application.
  - publish the Docker image to DockerHub

6. Manually run the workflow.

7. A group member who has not yet run the application should pull the image from DockerHub and run the image to verify the published image.
